    ...
    <div>
      ...
      <div style={{ maxWidth: 720, margin: '16px auto', padding: '0 16px' }}>
        ...
        <section style={cardStyle}>
          <div style={{ fontSize: 120, fontWeight: 'bold', lineHeight: 1, color: '#fb8500' }}>
            {item.letter}<span style={{ color: '#219ebc' }}>{item.lower}</span>
          </div>
          <div style={{ marginTop: 12 }}>
            <button onClick={() => speakEnglish(item.spokenName)} style={primaryBtn} aria-label={`朗读字母 ${item.letter}`}>
              🔊 读字母
            </button>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: 16 }}>
            {item.examples.map(ex => (
              <div key={ex.word} style={{
                ...
                <button onClick={() => speakEnglish(ex.word)} style={{
